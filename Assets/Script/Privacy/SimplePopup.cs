    ...
    public class SimplePopup : MonoBehaviour
    {
        ...
        void Start()
        {
            // ✅ Khi khởi động, kiểm tra xem có cần hiện hay không
            ShowIfNeeded();
        }
        ...
        void OnDecline()
        {
            ...
            Debug.Log("❌ Người chơi từ chối chính sách quyền riêng tư.");

            if (quitAppOnDecline)
            {
    #if UNITY_EDITOR
            Debug.LogWarning("⚠️ [Editor] Không thể thoát app — chỉ log ra thôi.");
            UnityEditor.EditorApplication.isPlaying = false; // tắt Play Mode trong Editor
    #else
            ...
            {
                Debug.Log("🚫 Không thoát app vì quitAppOnDecline = false.");
            }
        }
